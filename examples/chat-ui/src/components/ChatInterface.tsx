        ...
        <CardHeader className="border-b">
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg">
              {currentThreadId ? `Thread: ${currentThreadId.slice(0, 20)}...` : "New Conversation"}
            </CardTitle>
            <Button
              variant="outline"
              size="sm"
              onClick={() => currentThreadId && loadThreadMessages(currentThreadId)}
            >
              ...
              <div className="space-y-4">
                {messages.map((message) => (
                  <ChatMessage
                    ...
                    message={message}
                    streamingContent={streamingContent[message.message_id]}
                  />
                ))}
              </div>
            )}
